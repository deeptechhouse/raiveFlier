...
class InterconnectionService:
    ...
    async def analyze_streaming(
        ...
        identically.  Step 2 uses ``stream_complete()`` and yields each
        ...
        run identically.  The final result is yielded as an
        ``analysis_complete`` dict.

        Yields
        ------
        dict[str, Any]
            ``{"type": "narrative_chunk", "text": "<chunk>"}`` or
        ...
        # Steps 1-1.5: identical to analyze()
        compiled_context = self._compile_research_context(research_results)
        ...
        # Steps 3-4: parse, validate, enrich — requires full response

...
class TestArtistResearcher:
    ...
    def test_parse_event_appearances_standard(self):
        """Multi-line pipe-delimited input parses correctly."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        text = (
            "Festival X | Tresor | 1997-03-15 | https://ra.co/1\n"
            "Club Night | Berghain | unknown | https://ra.co/2"
        )
        appearances = researcher._parse_event_appearances(text)

        assert len(appearances) == 2
        assert appearances[0].event_name == "Festival X"
        assert appearances[0].venue == "Tresor"
        assert appearances[0].date == date(1997, 3, 15)
        assert appearances[1].event_name == "Club Night"
        assert appearances[1].venue == "Berghain"
        assert appearances[1].date is None  # "unknown" should not parse

    def test_parse_event_appearances_none_input(self):
        """Input containing 'NONE' returns empty list."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        assert researcher._parse_event_appearances("NONE") == []

    def test_parse_event_appearances_partial_lines(self):
        """Lines with fewer than 3 pipe-delimited parts are skipped gracefully."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        # Only 3 parts but all content-less -- still creates an appearance
        # with None fields because parts are empty strings
        appearances = researcher._parse_event_appearances(text)
        # The parser checks len(parts) < 3, this has 3 parts so it passes
        # but the fields will be None because empty strings fail the truthiness check
...
class TestArtistResearcherExtended:
    ...
    @pytest.mark.asyncio
    async def test_search_gig_history_basic(self):
        """Gig history search returns LLM-parsed event appearances."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="Carl Cox at Tresor",
                url="https://ra.co/events/123",
                snippet="Carl Cox played at Tresor in 1997.",
            ),
        ])
        llm = _mock_llm(
            response="Tresor Opening | Tresor | 1997-03-15 | https://ra.co/events/123"
        )
        ...
        appearances = await researcher._search_gig_history("Carl Cox", before_date=None, city=None)

        assert len(appearances) == 1
        assert appearances[0].event_name == "Tresor Opening"
        assert appearances[0].venue == "Tresor"

    @pytest.mark.asyncio
    async def test_search_gig_history_with_city(self):
        """Gig history search with city hint includes city-qualified query."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="Carl Cox Berlin",
                url="https://ra.co/events/456",
                snippet="Carl Cox in Berlin.",
            ),
        ])
        llm = _mock_llm(
            response="Berlin Night | Tresor | 1997-03-15 | https://ra.co/events/456"
        )
        ...
        appearances = await researcher._search_gig_history(
            "Carl Cox", before_date=None, city="Berlin"
        )

        assert len(appearances) == 1

    @pytest.mark.asyncio
    async def test_search_gig_history_empty_results(self):
        ...
        appearances = await researcher._search_gig_history("NobodyDJ", before_date=None)

        assert appearances == []

    @pytest.mark.asyncio
    async def test_search_gig_history_llm_failure(self):
        """LLM failure during gig extraction returns empty list."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        appearances = await researcher._search_gig_history("Carl Cox", before_date=None)

        assert appearances == []

    @pytest.mark.asyncio
    async def test_search_gig_history_deepen_search(self):
        """Fewer than 3 results triggers deeper search phase."""
        from src.services.artist_researcher import ArtistResearcher

        call_count = 0

        async def search_side_effect(query, num_results=10, before_date=None):
            nonlocal call_count
            call_count += 1
            if call_count <= 4:
                # Primary queries return only 1 result total
                return [_search_result(url=f"https://ra.co/{call_count}")]
            # Deeper queries return more
            return [_search_result(url=f"https://ra.co/deep/{call_count}")]

        web_search = _mock_web_search()
        web_search.search = AsyncMock(side_effect=search_side_effect)

        llm = _mock_llm(response="NONE")
        ...
        # Deeper queries should have been called (call_count >= 4)
        assert call_count >= 4

    @pytest.mark.asyncio
    async def test_search_gig_history_research_error_in_search(self):
        """ResearchError during web search is caught and skipped."""
        from src.services.artist_researcher import ArtistResearcher
        from src.utils.errors import ResearchError

        web_search = _mock_web_search()
        web_search.search = AsyncMock(side_effect=ResearchError("Search failed"))
    ...
    def test_parse_event_appearances_with_source_url(self):
        """Appearance with 4 pipe-delimited parts includes source_url."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        text = "Festival X | Tresor | 1997-03-15 | https://ra.co/events/123"
        appearances = researcher._parse_event_appearances(text)

        assert len(appearances) == 1
        assert appearances[0].source_url == "https://ra.co/events/123"

    def test_parse_event_appearances_unknown_values(self):
        """'unknown' event_name and venue are set to None."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        text = "unknown | unknown | 1997-03-15"
        appearances = researcher._parse_event_appearances(text)

        assert len(appearances) == 1
        assert appearances[0].event_name is None
        assert appearances[0].venue is None
...
class TestArtistResearcherCoverageFill:
    ...
    # --- lines 532-533: _search_gig_history deeper phase adds new URLs ---
    @pytest.mark.asyncio
    async def test_search_gig_history_deeper_adds_new_urls(self):
        """Deeper search phase adds results with new (unseen) URLs."""
        from src.services.artist_researcher import ArtistResearcher

        call_count = 0

        async def search_side_effect(query, num_results=30, before_date=None):
            nonlocal call_count
            call_count += 1
            # Primary phase: return only 1 unique result (below threshold of 3)
            if call_count <= 4:
                return [_search_result(url="https://ra.co/same")]
            # Deeper phase: return new unique URLs
            return [
                _search_result(url=f"https://ra.co/deeper/{call_count}"),
            ]
        ...
        llm = _mock_llm(
            response="Deep Event | Deep Venue | 2003-05-01 | https://ra.co/deeper/5"
        )
        ...
        appearances = await researcher._search_gig_history(
            "Carl Cox", before_date=None, city=None
        )

        # Deeper queries must have been called
        assert call_count > 4
        # LLM received results including deeper URLs
        assert len(appearances) >= 1

    # --- lines 643-742: _search_press full flow ---
    @pytest.mark.asyncio
    async def test_search_press_happy_path(self):
        """Press search finds results, filters relevance, extracts article refs."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="Carl Cox interview Resident Advisor",
                url="https://ra.co/features/carl-cox",
                snippet="Carl Cox DJ interview",
            ),
            _search_result(
                title="Carl Cox Mixmag",
                url="https://mixmag.net/carl-cox",
                snippet="Carl Cox mix release",
            ),
        ])
        ...
        article_scraper = _mock_article_scraper(
            content=_article_content(
                ...
                text="Carl Cox spoke about his career in techno. Carl Cox loves vinyl.",
            )
        )
        ...
        articles = await researcher._search_press("Carl Cox", before_date=None, city=None)

        assert len(articles) >= 1
        # Articles should have ArticleReference objects
        assert all(isinstance(a, ArticleReference) for a in articles)

    @pytest.mark.asyncio
    async def test_search_press_with_city_adds_query(self):
        """City hint adds a city-qualified query to press search."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="Carl Cox Berlin scene",
                url="https://ra.co/features/carl-cox-berlin",
                snippet="Carl Cox DJ Berlin techno",
            ),
        ])
        ...
        article_scraper = _mock_article_scraper(
            content=_article_content(
                title="Carl Cox Berlin",
                text="Carl Cox played in Berlin many times.",
            )
        )
        ...
        articles = await researcher._search_press(
            "Carl Cox", before_date=None, city="Berlin"
        )

        # City-qualified query should have been added (4 queries instead of 3)
        assert web_search.search.await_count >= 4

    @pytest.mark.asyncio
    async def test_search_press_deepens_when_sparse(self):
        """Fewer than 2 relevant results triggers deeper press search."""
        from src.services.artist_researcher import ArtistResearcher

        call_count = 0

        async def search_side_effect(query, num_results=20, before_date=None):
            nonlocal call_count
            call_count += 1
            if call_count <= 3:
                return []  # Primary queries return nothing
            # Deeper queries return results
            return [
                _search_result(
                    title="DJ Carl Cox feature",
                    url=f"https://xlr8r.com/deeper/{call_count}",
                    snippet="Carl Cox is a DJ and producer",
                ),
            ]
        ...
        researcher = ArtistResearcher(
            ...
            article_scraper=_mock_article_scraper(
                content=_article_content(
                    title="Carl Cox XLR8R", text="Carl Cox feature article."
                )
            ),
            llm=_mock_llm(),
            cache=_mock_cache(),
        ...
        articles = await researcher._search_press("Carl Cox", before_date=None)

        # Deeper queries should have been triggered
        assert call_count > 3

    @pytest.mark.asyncio
    async def test_search_press_filters_irrelevant(self):
        """Non-music-relevant results are filtered out of press results."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="Carl Cox tax returns",
                url="https://finance.example.com/carlcox",
                snippet="Carl Cox financial records disclosed.",
            ),
            _search_result(
                title="Carl Cox DJ Mag Top 100",
                url="https://djmag.com/carl-cox",
                snippet="Carl Cox DJ Mag feature",
            ),
        ])
        ...
        researcher = ArtistResearcher(
            ...
            article_scraper=_mock_article_scraper(
                content=_article_content(
                    title="Carl Cox DJ Mag", text="Carl Cox is a top DJ."
                )
            ),
            llm=_mock_llm(),
            cache=_mock_cache(),
        ...
        # Finance result should be filtered out (no music terms), DJ Mag should pass
        assert len(articles) >= 1
        urls = [a.url for a in articles]
        assert "https://finance.example.com/carlcox" not in urls

    @pytest.mark.asyncio
    async def test_search_press_no_results(self):
        """No search results returns empty article list."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        articles = await researcher._search_press("Unknown DJ", before_date=None)

        assert articles == []

    @pytest.mark.asyncio
    async def test_search_press_all_filtered_returns_empty(self):
        """All search results fail relevance check returns empty."""
        from src.services.artist_researcher import ArtistResearcher

        web_search = _mock_web_search(results=[
            _search_result(
                title="John Smith plumber",
                url="https://plumber.example.com/john",
                snippet="John Smith fixes pipes.",
            ),
        ])
        ...
        articles = await researcher._search_press("John Smith", before_date=None)
    ...
    # --- lines 804/808: _parse_event_appearances edge cases ---
    def test_parse_event_appearances_line_with_dash_prefix(self):
        """Lines with '- ' prefix are stripped correctly."""
        from src.services.artist_researcher import ArtistResearcher
        ...
        text = "- Festival X | Tresor | 1997-03-15\n- | | "
        appearances = researcher._parse_event_appearances(text)

        # First line has 3 parts, valid
        assert len(appearances) >= 1
        assert appearances[0].event_name == "Festival X"

    def test_parse_event_appearances_line_without_pipe(self):
        ...
        text = "No pipe here at all\nAnother line without pipe"
        appearances = researcher._parse_event_appearances(text)

        assert appearances == []

    def test_parse_event_appearances_two_part_line_skipped(self):
        """Lines with only 2 pipe-delimited parts are skipped (< 3)."""
        from src.services.artist_researcher import ArtistResearcher
    ...
    # --- lines 935-942: _synthesize_profile with appearances and articles ---
    @pytest.mark.asyncio
    async def test_synthesize_profile_with_appearances_and_articles(self):
        """Profile synthesis includes event names and article titles in context."""
        from src.services.artist_researcher import ArtistResearcher

        llm = _mock_llm(response="Carl Cox is a pioneering techno DJ.")
        ...
        summary = await researcher._synthesize_profile(
            ...
            releases=[Release(title="Phat Trax", label="Intec", year=1995)],
            appearances=[
                EventAppearance(event_name="Tresor Opening", venue="Tresor"),
                EventAppearance(event_name="Love Parade", venue="Tiergarten"),
                EventAppearance(event_name=None),  # None event_name, should be filtered
            ],
            articles=[
                ArticleReference(
                    title="Carl Cox RA Feature",
                    source="ra.co",
                    url="https://ra.co/features/1",
                    citation_tier=1,
                ),
                ArticleReference(
                    title="Untitled",  # Low-quality article
                    source="unknown",
                    citation_tier=6,
                ),
            ],
        ...
        assert summary is not None
        # Verify the LLM was called with context including events and articles
        call_args = llm.complete.call_args
        user_prompt = call_args.kwargs.get("user_prompt", call_args[1].get("user_prompt", ""))
        assert "Tresor Opening" in user_prompt or "Events:" in user_prompt
